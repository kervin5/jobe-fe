...
const EditJobForm = ({ data, jobId }) => {
  const [updateJobMutation, mutationState] = useMutation(UPDATE_JOB_MUTATION);
  ...
  return (
    <Paper className="Body" >
      ...
      <StyledForm
        ...
        aria-disabled={mutationState.loading || !!mutationState.data}
      >
...
const EditJobFormDetail = ({ jobId }) => {
  const { error, loading, data } = useQuery(SINGLE_JOB_ALL_DATA_QUERY, {
    variables: { id: jobId },
  });
